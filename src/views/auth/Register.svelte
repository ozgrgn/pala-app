<script>
  ...
  const register = async () => {
    ...
    if (customerResponse && customerResponse.status) {
      ...
      companyId = customerResponse["customer"]._id;
      if (companyId) {
        userRegister();
      }
    } else {
  ...
  const userRegister = async () => {
    let editedPhone = dialCode + phone.value.replace(/\s/g, "");
...
<div class="container mx-auto ">
  <div class="lg:flex gap-6">
    ...
    {:else}
    <div class="container text-center items-center  h-screen mx-auto flex flex-col justify-center">
      <img class="h-28 w-fit py-4 " src="/assets/img/logos/logo-light.png" alt="" />
    Bayi kaydınız alınmıştır.<br> Kaydınız incelendikten sonra bilgilendirileceksiniz. <br> Desteğe ihtiyacınız varsa lütfen <span> <a href="mailto:info@pala.com"> info@pala.com</a> </span> adresine iletin.
    </div>
    {/if}
  </div>
</div>
