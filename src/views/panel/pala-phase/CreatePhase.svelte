<script>
  ...
  values.map((v) => {
    if (v.defaultValue) {
      phase[v.key] = { value: v.defaultValue };
    } else {
      phase[v.key] = {};
    }
  });
  ...
  const addPhase = async () => {
    let data = {};
    values.map((v) => {
      if (phase[v.key].value) {
      data[v.key] = phase[v.key]?.value;}
    });
    let response = await RestService.addPhase(data);
    if (response["status"]) {
      ...
      <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
        ...
        <div class="flex flex-wrap">
          <div class="w-full lg:w-12/12 px-4 text-right mt-2">
            <button
              on:click={addPhase}
              disabled={!phase.name.isValid || phase.name.value == null}
