<script>
  ...
  const updateUnit = async () => {
    ...
    values.map((v) => {
      editedUnit[v.key] = unit[v.key].value;
    });

    let response = await RestService.updateUnit(unit._id, editedUnit);
    if (response["status"]) {
      ToastService.success($Translate("Successfully-completed"));
      navigate("/panel/units");
    } else {
  ...
  const getUnit = async () => {
    let response = await RestService.getUnit($params.unitId);

    if (response["status"]) {
      values.map((v) => {
        if (v.customValue) {
          response["unit"][v.key] = {
            value: response["unit"][v.key][v.customValue],
          };
        } else {
          response["unit"][v.key] = { value: response["unit"][v.key] };
        }
      });
      unit = {
        ...response["unit"],
  ...
  <div class="w-full mb-12 px-2 lg:px-4 ">
    <div class="flex justify-between">
      ...
      <button
        class="bg-white text-blue-600 hover:text-red-700 mb-2 border rounded font-bold text-xs px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 "
        type="button"
        on:click={() => deleteUnitApprove($params.unitId)}
      >
        <i class="fa fa-trash" />
        Sil
      </button>
        ...
        <div class="flex flex-wrap my-4">
          <div class="w-full lg:w-3/12 px-4">
            <div class="relative w-full mb-3">
              ...
              <Input
                bind:value={unit.name.value}
                bind:isValid={unit.name.isValid}
          ...
          <div class="w-full lg:w-7/12 px-4">
            <div class="relative w-full mb-3">
              ...
              <Textarea
                bind:value={unit.note.value}
                bind:isValid={unit.note.isValid}
          ...
          <div class="w-full lg:w-2/12 px-4">
            <div class="relative w-full mb-3">
              ...
              <Switch bind:value={unit.isActive.value} />
            </div>
          </div>
